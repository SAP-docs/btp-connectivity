...
> ### Note:  
> If you use your own CA certificates for the e-mail configuration \(see [Alerting](alerting-87bffd9.md)\) or for LDAP \(see [Use LDAP for User Administration](use-ldap-for-user-administration-120ceec.md)\), and you imported these custom CA certificates into your JVM trust store, you must reimport them after a JVM update into the JVM trust store as described there.

> ### Tip:  
> If own CA certificates for e-mail alerting or LDAP user authorization setup are required, but these certificates are not necessary for other Java processes using the very same JVM installation, consider storing these custom CA certificates in the respective trust stores of the Cloud Connector instead of importing them into the JVM trust store. In this case, you do not need to reimport them into the JVM trust store after a JVM update, and you will benefit from these certificates being stored in Cloud Connector configuration backups.
